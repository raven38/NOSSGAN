...
from utils.sample import closedset_class_indices
...
def prepare_train_eval(local_rank, gpus_per_node, world_size, run_name, train_configs, model_configs, hdf5_path_train):
    ...
    if cfgs.distributed_data_parallel:
        ...
        train_sampler = None
        if cfgs.conditional_strategy in ['ProjGAN']:
            labeled_ratio = 1.0
        else:            
            labeled_ratio = cfgs.batch_ratio if hasattr(cfgs, 'batch_ratio') else 0.5
        try:
    ...
    eval_dataloader = DataLoader(eval_dataset, batch_size=cfgs.batch_size, sampler=closedset_class_indices(eval_dataset), shuffle=False, pin_memory=True, num_workers=cfgs.num_workers, drop_last=True)
    ...
    if cfgs.eval or cfgs.intra_class_fid:
    ...
    if cfgs.intra_class_fid:
        worker.run_intra_class_fid(standing_statistics=cfgs.standing_statistics, standing_step=cfgs.standing_step)
