...
def loss_hinge_dis(dis_out_real, dis_out_fake, weight_real=None):
    if weight_real is None:
        return torch.mean(F.relu(1. - dis_out_real)) + torch.mean(F.relu(1. + dis_out_fake))
    else:
        real = F.relu(1. - dis_out_real)
        weighted_real = len(dis_out_real) * torch.mean(real * weight_real)
        fake = torch.mean(F.relu(1. + dis_out_fake))
