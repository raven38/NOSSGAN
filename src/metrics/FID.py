...
def generate_images(batch_size, gen, dis, truncated_factor, prior, latent_op, latent_op_step,
                    latent_op_alpha, latent_op_beta, device, sampler):
    ...
    zs, fake_labels = sample_latents(prior, batch_size, z_dim, truncated_factor, num_classes, None, device, sampler)
...
def get_activations(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior, is_generate,
                    latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable=False, sampler="default", run_name=None):
...
def calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                                    is_generate, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable, sampler="default", run_name=None):
    act = get_activations(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                          is_generate, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable, sampler, run_name)
...
def calculate_fid_score(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                        latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, logger, sampler="default", pre_cal_mean=None, pre_cal_std=None, run_name=None):
    ...
    if pre_cal_mean is not None and pre_cal_std is not None:
        ...
        m1, s1 = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor,
                                                 prior, False, False, 0, latent_op_alpha, latent_op_beta, device, tqdm_disable=disable_tqdm, sampler=sampler)
    ...
    m2, s2 = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                                             True, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable=disable_tqdm, sampler=sampler, run_name=run_name)
