...
from metrics.ClassAccuracy import calculate_class_accuracy
...
from utils.losses import Conditional_Contrastive_loss, Proxy_NCA_loss, NT_Xent_loss, HLoss, GeneralizedCELoss
...
class make_worker(object):
    def __init__(self, cfgs, train_configs, model_configs, run_name, best_step, logger, writer, n_gpus, gen_model, dis_model,
        ...
        self.ent_loss = HLoss()
        self.gce_loss = GeneralizedCELoss(q=0.7)
        ...
        if self.conditional_strategy in ['SSGAN', 'OSSSGAN', 'NOSSGAN', 'ProjGAN', 'ContraGAN', 'Proxy_NCA_GAN', 'Random', 'Reject', 'Single']:
        ...
        elif self.dataset_name in ["cifar10", "cifar100", "nosscifar10", "nosscifar100"]:
        ...
        if hasattr(cfgs, 'rc') and cfgs.rc and self.train_dataset.noise_rate > 0:
            if isinstance(self.train_dataset.data, np.ndarray):
                self.T = (torch.eye(self.train_dataset.num_closedset_classes) * (1 - self.train_dataset.noise_rate) +
                          (torch.ones([self.train_dataset.num_closedset_classes, self.train_dataset.num_closedset_classes]) /
                           self.train_dataset.num_closedset_classes * self.train_dataset.noise_rate)).to(self.local_rank)
            else:
                self.T = self.train_dataset.data.T(self.train_dataset.noise_type, self.train_dataset.noise_rate).to(self.local_rank)
            self.noise2clean = functools.partial(noise2clean, T=self.T)
        else:
            self.T = None
            self.noise2clean = lambda x: x
            ...
            for step_index in range(self.d_steps_per_iter):
                ...
                for acml_index in range(self.accumulation_steps):
                    ...
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        fake_labels_clean = self.noise2clean(fake_labels)
                        if self.latent_op:
                            zs = latent_optimise(zs, fake_labels_clean, self.gen_model, self.dis_model, self.conditional_strategy,
                                                 self.latent_op_step, self.latent_op_rate, self.latent_op_alpha, self.latent_op_beta,
                                                 False, self.local_rank)

                        fake_images = self.gen_model(zs, fake_labels_clean)
                        if self.diff_aug:
                        ...
                        if self.conditional_strategy in ["ACGAN"]:
                            ...
                            cls_out_real, dis_out_real = self.dis_model(real_images, real_labels)
                            cls_out_fake, dis_out_fake = self.dis_model(fake_images, fake_labels)
                        elif self.conditional_strategy in ["NOSSGAN"]:
                            cls_out_real, dis_out_real = self.dis_model(real_images, real_labels)
                            cls_out_fake, dis_out_fake = self.dis_model(fake_images, fake_labels)
                            weight_real = F.softmax(1 - calc_entropy(cls_out_real).detach(), dim=0).detach()
                        ...
                        dis_acml_loss = self.D_loss(dis_out_real, dis_out_fake, weight_real=weight_real)
                        ce = None
                        if self.conditional_strategy == "ACGAN":
                            if self.T is None:
                                ce_real = self.ce_loss(cls_out_real, real_labels)
                            else:
                                eps = 1e-8
                                p = F.softmax(cls_out_real, dim=1)
                                ce_real = - torch.sum(torch.eye(self.num_classes)[real_labels]
                                                      * torch.log(p.mm(self.T) + eps)) / real_labels.size(0)
                            ce_fake = self.ce_loss(cls_out_fake, fake_labels_clean)
                            dis_acml_loss += (ce_real + ce_fake)
                        elif self.conditional_strategy in ["SSGAN"]:
                            ...
                            if self.T is None:
                                ...
                                eps = 1e-8
                                p = F.softmax(cls_out_real, dim=1)
                                ce = - torch.sum(torch.eye(self.num_classes, device=self.local_rank)[real_labels.index_select(0, mask)]
                                                 * torch.log(p.index_select(0, mask).mm(self.T) + eps)) / real_labels.index_select(0, mask).size(0)
                            dis_acml_loss += self.cross_entropy_lambda * ce
                        elif self.conditional_strategy in ["OSSSGAN", 'Open', 'Reject']:
                            mask = torch.nonzero(real_labels != -1).squeeze()
                            ratio = mask.shape[0] / self.batch_size
                            if self.T is None:
                                ce_real = self.ce_loss(cls_out_real.index_select(0, mask), real_labels.index_select(0, mask))
                            else:
                                eps = 1e-8
                                p = F.softmax(cls_out_real, dim=1)
                                ce_real = - torch.sum(torch.eye(self.num_classes, device=self.local_rank)[real_labels.index_select(0, mask)]
                                                      * torch.log(p.index_select(0, mask).mm(self.T) + eps)) / real_labels.index_select(0, mask).size(0)
                            ce_fake = self.ce_loss(cls_out_fake, fake_labels_clean) * ratio
                            ent = self.ent_loss(cls_out_real) + self.ent_loss(cls_out_fake) * ratio
                            dis_acml_loss += self.cross_entropy_lambda * ((ce_real + ce_fake) - ent)
                        elif self.conditional_strategy in ["NOSSGAN"]:
                            mask = torch.nonzero(real_labels != -1).squeeze()
                            ratio = mask.shape[0] / self.batch_size
                            ce_real = self.gce_loss(cls_out_real.index_select(0, mask), real_labels.index_select(0, mask))
                            ce_fake = self.gce_loss(cls_out_fake, fake_labels) * ratio
                            # ent = self.ent_loss(cls_out_real) + self.ent_loss(cls_out_fake) * ratio
                            # dis_acml_loss += self.cross_entropy_lambda * ((ce_real + ce_fake) - ent)
                            dis_acml_loss += self.cross_entropy_lambda * (ce_real + ce_fake) # - ent)
                        elif self.conditional_strategy == "NT_Xent_GAN":
                        ...
                        if self.cr:
                            ...
                            elif self.conditional_strategy in ["SSGAN", "OSSSGAN", "NOSSGAN", "ProjGAN", "no", 'Random', 'Single', 'Open', 'Reject']:
                        ...
                        if self.zcr:
                            fake_images_zaug = self.gen_model(zs_t, fake_labels_clean)
            ...
            for step_index in range(self.g_steps_per_iter):
                ...
                for acml_step in range(self.accumulation_steps):
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        fake_labels_clean = self.noise2clean(fake_labels)
                        if self.latent_op:
                            zs, transport_cost = latent_optimise(zs, fake_labels_clean, self.gen_model, self.dis_model, self.conditional_strategy,
                        ...
                        if self.conditional_strategy in ["ACGAN"]:
                            gen_acml_loss += self.ce_loss(cls_out_fake, fake_labels_clean)
                        elif self.conditional_strategy == "ContraGAN":
    ...
    def evaluation(self, step, standing_statistics, standing_step):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            fid_score, self.m1, self.s1 = calculate_fid_score(self.eval_dataloader, generator, self.dis_model, self.inception_model, self.num_eval[self.eval_type],
                                                              ...
                                                              self.latent_op_beta, self.local_rank, self.logger, 'default', self.mu, self.sigma, self.run_name)
            ...
            if self.D_loss.__name__ != "loss_wgan_dis":
                ...
                real_label_clsacc, real_unlabel_clsacc, fake_clsacc = calculate_class_accuracy(self.train_dataloader, generator, self.dis_model, self.D_loss, self.num_eval[self.eval_type],
                                                                          self.truncated_factor, self.prior, self.latent_op, self.latent_op_step, self.latent_op_alpha,
                                                                          self.latent_op_beta, self.local_rank, cr=self.cr, logger=self.logger, eval_generated_sample=True)

                if self.eval_type == 'train':
                    acc_dict = {'real_accuracy_label': real_label_clsacc, 'real_accuracy_unlabel': real_unlabel_clsacc, 'fake_accuracy': fake_clsacc}
                else:
                    real_eval_clsacc = calculate_class_accuracy(self.eval_dataloader, generator, self.dis_model, self.D_loss, self.num_eval[self.eval_type],
                                                                self.truncated_factor, self.prior, self.latent_op, self.latent_op_step, self.latent_op_alpha,
                                                                self.latent_op_beta, self.local_rank, cr=self.cr, logger=self.logger, eval_generated_sample=False)
                    clsacc_dict = {'real_accuracy_label': real_label_clsacc, 'real_accuracy_unlabel': real_unlabel_clsacc, 'real_accuracy_valid': real_eval_clsacc, 'fake_accuracy': fake_clsacc}

                if self.global_rank == 0: self.writer.add_scalars('Class Accuracy', clsacc_dict, step)
            ...
            if self.global_rank == 0:
                ...
                self.logger.info('Classification accuracy for real label (Step: {step}): {acc}'.format(step=step, acc=real_label_clsacc))
                self.logger.info('Classification accuracy for real unlabel (Step: {step}): {acc}'.format(step=step, acc=real_unlabel_clsacc))
                self.logger.info('Classification accuracy for fake (Step: {step}): {acc}'.format(step=step, acc=fake_clsacc))
                if self.eval_type != 'train':
                    self.logger.info('Classification accuracy for real eval (Step: {step}): {acc}'.format(step=step, acc=real_eval_clsacc))
                if self.train:
    ...
    # -----------------------------------------------------------------------------
    def run_intra_class_fid(self, standing_statistics, standing_step):
        if self.global_rank == 0:
            self.logger.info("Start calculating iFID (use approx. {num} fake images per class and train images as the reference).".format(num=int(len(self.eval_dataset.data)/self.num_classes)))
        if standing_statistics: self.counter += 1
        fids = []
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            self.dis_model.eval()
            generator = change_generator_mode(self.gen_model, self.Gen_copy, self.bn_stat_OnTheFly, standing_statistics, standing_step,
                                              self.prior, self.batch_size, self.z_dim, self.num_classes, self.local_rank, training=False, counter=self.counter)
            for c in tqdm(range(self.num_classes)):
                num_samples, target_sampler = target_class_sampler(self.eval_dataset, c)
                batch_size = self.batch_size if num_samples >= self.batch_size else num_samples
                dataloader = torch.utils.data.DataLoader(self.eval_dataset, batch_size=batch_size, shuffle=False, sampler=target_sampler,
                                                         num_workers=self.num_workers, pin_memory=True, drop_last=False)
                fid_score, _, _ = calculate_fid_score(dataloader, generator, self.dis_model, self.inception_model, self.num_eval[self.eval_type],
                                                      self.truncated_factor, self.prior, self.latent_op, self.latent_op_step4eval, self.latent_op_alpha,
                                                      self.latent_op_beta, self.local_rank, self.logger, c, None, None, self.run_name)
                fids.append(fid_score)

            if self.global_rank == 0:
                self.logger.info("Average iFID score: {iFID}".format(iFID=sum(fids, 0.0) / len(fids)))

            save_array_npy(join('./statistics', self.run_name), 'iFID', np.array(fids))
